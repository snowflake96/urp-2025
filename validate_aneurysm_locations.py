...
def process_patient(patient_id, patient_data, input_dir, output_dir):
...
def main():
    ...
    def process_wrapper(patient_item):
        patient_id, patient_data = patient_item
        return process_patient(patient_id, patient_data, input_dir, output_dir)
    
    print(f"Processing {len(aneurysm_data)} patients with {args.workers} workers...")
    results = Parallel(n_jobs=args.workers)(
        delayed(process_wrapper)(patient_item) 
        for patient_item in aneurysm_data.items()
    )
